...
unsigned char numberTable[17+8] = {
	0x3F, 0x06, 0x5B, 0x4F, 0x66, 0x6D, 0x7D, 0x07, 0x7F, 0x6F, 
	0x77, 0x7C, 0x39, 0x5E, 0x79, 0x71, 0x00, 
	// 单笔画
	0x20, 0x10, 0x01, 0x02, 0x04, 0x40, 0x08, 0x80
};
/**
...
/**
 * @brief: 在LED上的第line行的第col列显示浮点数float，其中整数部分显示int_part个数字，小数部分显示float_part个数字。加上符号和小数点，一共是float+int+2。
 * @param {unsigned char} line 显示在第几行，1-2
 * @param {unsigned char} col  显示在第几列，1-16
 * @param {float} num  显示的浮点数
 * @param {unsigned char} int_part 显示的整数位
 * @param {unsigned char} float_part  显示的小数位
 * @return {*} 无
 */
void LCD_ShowFloat(unsigned char line, unsigned char col, float num, unsigned char int_part, unsigned char float_part)
{
	unsigned char i;
	...
	LCD_ShowNum(line, col+1, num, int_part);
	LCD_ShowChar(line, col+1+int_part, '.');
	
	for(i=0; i<float_part; i++){
		scale = scale * 10; 
	}
	LCD_ShowNum(line, col+1+int_part+1, ((unsigned long) (scale * num) % scale), float_part);
}
